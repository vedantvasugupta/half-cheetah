...
class ReplayBuffer(object):
    ...
    def store_transition(self, state,action,reward,state_ , done):
        ...
        self.terminal_memory[index]= 1-done #done is boolean
        self.mem_cntr += 1
...
class Actor(object):
    def __init__(self, lr, n_actions, name, input_dims, sess, fc1_dims,
        ...
        self.checkpoint_file = os.path.join(chkpt_dir, name + '_ddpg.ckpt').replace("\\","/")
...
class Critic(object):
    def __init__(self, lr, n_actions, name, input_dims, sess, fc1_dims, fc2_dims, batch_size = 64, chkpt_dir='tmp/ddpg'):
        ...
        self.params = tf.trainable_variables(scope=self.name)
        self.saver = tf.train.Saver()
        self.checkpoint_file = os.path.join(chkpt_dir, name+'_.ckpt').replace("\\","/")
    ...
    def train(self, inputs, actions, q_target):
        self.session.run(self.optimize, feed_dict={self.input: inputs, self.actions: actions, self.q_target: q_target})
    
    def get_action_gradients(self, input,actions):
        return self.session.run(self.action_gradients, feed_dict={self.input: input, self.actions: actions})
...
class Agent(object):
    def __init__(self, alpha, beta, input_dims, tau, env, gamma=0.99, n_actions = 2, max_size = 1000000, layer1_size =400, layer2_size = 300, batch_size = 64, chkpt_dir = 'tmp/ddpg'):
        ...
        self.actor = Actor(alpha, n_actions, 'Actor', input_dims, self.session,
                           layer1_size, layer2_size, env.action_space.high,
                            chkpt_dir=chkpt_dir)
        self.critic = Critic(beta, n_actions, 'Critic', input_dims, self.session, layer1_size, layer2_size, chkpt_dir)
        self.target_critic = Critic(beta, n_actions, 'TargetCritic', input_dims, self.session, layer1_size, layer2_size, chkpt_dir)
        self.target_actor = Actor(alpha, n_actions, 'TargetActor', input_dims, self.session, layer1_size, layer2_size, env.action_space.high[0], chkpt_dir=chkpt_dir)

        self.noise = OUActionNoise(mu=np.zeros(n_actions))

        self.update_critic = \
            [self.target_critic.params[i].assign(tf.multiply(self.critic.params[i], self.tau) + tf.multiply(self.target_critic.params[i], 1. - self.tau)) for i in range(len(self.target_critic.params))]
        self.update_actor = \
            [self.target_actor.params[i].assign(tf.multiply(self.actor.params[i], self.tau) + tf.multiply(self.target_actor.params[i], 1. - self.tau)) for i in range(len(self.target_actor.params))]
    ...
    def remember(self, state, action,reward, action_ , done):
        self.memory.store_transition(state, action, reward, action_, done)
